...
//            }};
    ...
    public static AllExpression priority1(ArrayList<TaskElement> taskElements) {
        ...
        while (true) {
            ...
            } else if (taskElement.type == TaskElementType.ACTION_COUNT) {
                return new CheckedCount(priority1(taskElements));
    ...
    public static AllExpression priority4(ArrayList<TaskElement> taskElements) {
        ...
        while (true) {
            kur++;
            TaskElement taskElement = taskElements.get(kur);
            if (taskElement.type == TaskElementType.ACTION_SET) {
    ...
    public static ArrayList<TaskElement> lexAnalysis(String task) {
        ...
        actions.put("count", TaskElementType.ACTION_COUNT);
        int balance = 0;
        ...
        for (int i = 0; i < task.length(); ) {
            ...
            if (taskElementType == TaskElementType.NONE) {
                if (el <= '9' && el >= '0') {
                    ...
                    if (LongOperations.contains(taskElementType) && (i > 0 && task.charAt(i - 1) >= '0' && task.charAt(i - 1) <= '9'
...
//        e.parse("-2147483647 * -1");
        System.out.println(e.parse("count(1)\n").evaluate(0, 0, 0));
//        for (int i = 0; i < 11; i++) {
//            try {
